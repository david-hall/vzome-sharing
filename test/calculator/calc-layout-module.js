import { CalcLayoutCSS } from "./vzome-calculator.css.js";

export class CalcLayout extends HTMLElement {
	#root;
	#container;
	#model;
	...
	constructor() {
		...
		this.#root.appendChild( document.createElement("style") ).textContent = CalcLayoutCSS;
	...
	initializeStore() {
		...
		this.#model = new CalcLayoutController(fieldName, sharedStore);
			...
			function (event) {
				...
				switch(value) {
					case "":
						return;
					case "swap-operands":
						event.srcElement.closest("div.calculator").parentNode.host.doAction("swap-operands");
						break;
					case "info":
						alert("\n" + fieldName + "\n\norder " + order);
						break;
	...
	appendTdfCells(tr, tdf, asInput) {
				...
				function (event) {
					...
					if(event.srcElement.checkValidity()) {
						event.srcElement.closest("div.calculator").parentNode.host.setOperand(
							...
							event.srcElement.closest("td").getAttribute("data-col"),
							event.target.value);
					} else {
		...
		if(asInput) {
			...
			function (event) {
				event.srcElement.setCustomValidity("");
				if(event.srcElement.checkValidity()) {
					event.srcElement.closest("div.calculator").parentNode.host.setDivisor(
						event.srcElement.closest("tr").getAttribute("data-row"), 
						event.target.value);
				} else {
	...
	getOperationPickList(selection) {
		...
		CalcLayout.operationMap.forEach((label, value) => {
			if(validOperations.includes(value)) {
				const option = document.createElement("option");
				option.value = value;
				option.selected = (value == selection);
				option.label = CalcLayout.htmlDecode(label);
		...
		selectList.addEventListener('change', function (event) {
			event.srcElement.closest("div.calculator").parentNode.host.operator = event.target.value;
		});
		return selectList;
	}
...
`
	...
	static #unaryOperators = new Map([
		...
		[ "negate", 		"- #"	],
		[ "reciprocal",		"1 / #"	],
		[ "unit-exponent",	CalcLayout.htmlDecode( "&#x0023 &#x00B9") ], // superscript 1 as exponent
		[ "absolute-value",	CalcLayout.htmlDecode( "&#x23D0 &#x0023 &#x23D0" ) ],
		[ "ceiling",		CalcLayout.htmlDecode( "&#x2308 &#x0023 &#x2309" ) ],
		[ "floor",			CalcLayout.htmlDecode( "&#x230A &#x0023 &#x230B" ) ],
	]);
	...
	getRowPickList() {
		...
		//
		CalcLayout.#unaryOperators.forEach((label, value) => {
			const option = document.createElement("option");
			option.value = value;
			option.label = label;
			if(value == "=") {
		...
		selectList.addEventListener('change', function (event) {
			...
			const pickList = event.srcElement;
	...
	static operationMap = new Map([
		["add",              	CalcLayout.htmlDecode( "&plus;"  ) ],
		["subtract",         	CalcLayout.htmlDecode( "&minus;" ) ],
		["multiply",         	CalcLayout.htmlDecode( "&times;" ) ],
		["divide",           	CalcLayout.htmlDecode( "&div;"   ) ],
		["modulus",          	CalcLayout.htmlDecode( "&#xFF05;") ],
		["continued-fraction",	"///" ],
	]);
...
customElements.define( "vzome-calculator", CalcLayout );