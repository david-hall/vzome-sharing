import { reduxCalculatorCSS } from "./redux-calculator.css.js";
import { ReduxCalculatorModel } from "./redux-calculator-model.js";

export class ReduxCalculator extends HTMLElement {
	#root;
	#container;
	#model;
	...
	constructor() {
		...
		this.#root.appendChild( document.createElement("style") ).textContent = reduxCalculatorCSS;
	...
	#initializeStore() {
		...
		this.#model = new ReduxCalculatorModel(fieldName, sharedStore);
	...
	appendAlgebraicNumber(tr, format, num, asInput) {
		...
		switch(format) {
		case "alg":
			this.appendTdDiv(tr, format, num.alg);
			break;
		case "dec":
			this.appendTdDiv(tr, format, num.dec);
			break;
		case "tdf":
			this.appendTdfCells(tr, num.tdf, asInput);
			break;
		default:
	...
	appendTdfCells(tr, tdf, asInput) {
		if(tdf.isInfinite || !Array.isArray(tdf)) {
			// may be overkill...
	...
	getOperationPickList(selection) {
		...
		ReduxCalculator.operationMap.forEach((label, value) => {
			if(validOperations.includes(value)) {
				const option = document.createElement("option");
				option.value = value;
				option.selected = (value == selection);
				option.label = ReduxCalculator.htmlDecode(label);
...
<select class='menu'>
	...
	<option label='C' value='clear'></option>
	<option label='&#x21F5;' value='swap'></option>
	<option label='?' value='info'></option>
</select>
`
	}
	...
	static #unaryOperators = new Map([
		...
		[ "negate", 		"- #"	],
		[ "reciprocal",		"1 / #"	],
		[ "unit-exponent",	ReduxCalculator.htmlDecode( "&#x0023 &#x00B9") ], // superscript 1 as exponent
		[ "absolute-value",	ReduxCalculator.htmlDecode( "&#x23D0 &#x0023 &#x23D0" ) ],
		[ "ceiling",		ReduxCalculator.htmlDecode( "&#x2308 &#x0023 &#x2309" ) ],
		[ "floor",			ReduxCalculator.htmlDecode( "&#x230A &#x0023 &#x230B" ) ],
	]);
	...
	getRowPickList() {
		...
		//
		ReduxCalculator.#unaryOperators.forEach((label, value) => {
			const option = document.createElement("option");
			option.value = value;
	...
	attributeChangedCallback( attributeName, _oldValue, _newValue ) {
		if(this.#store) {
			...
			case "field-name":
				this.#store.dispatch( { type: 'field-name-change', payload: _newValue } );
				return;
			case "operator":
				this.#store.dispatch( { type: 'operator-change', payload: _newValue } );
				return;
			case "format":
				this.#store.dispatch( { type: 'format-change', payload: _newValue } );
				return;
			default:
	...
	static operationMap = new Map([
		["add",              	ReduxCalculator.htmlDecode( "&plus;"  ) ],
		["subtract",         	ReduxCalculator.htmlDecode( "&minus;" ) ],
		["multiply",         	ReduxCalculator.htmlDecode( "&times;" ) ],
		["divide",           	ReduxCalculator.htmlDecode( "&div;"   ) ],
		["modulus",          	ReduxCalculator.htmlDecode( "&#xFF05;") ],
		["continuedFraction",	"///" ],
	]);
...
customElements.define( "redux-calculator", ReduxCalculator );